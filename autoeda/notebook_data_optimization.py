    ...
    for col in df.columns:
        if any(x in col.lower() for x in ['date', 'time', 'timestamp']):
            if df[col].dtype == 'object': # Only attempt conversion if it's an object type
                processed_date_cols.add(col) # Mark as processed for date conversion attempt
                try:
                    logger.info(f"Attempting to convert column '{col}' to datetime objects.")
                    df[col] = pd.to_datetime(df[col])
                    logger.info(f"Successfully converted column '{col}' to datetime.")
                except Exception as e:
                    logger.warning(f"Could not convert column '{col}' to datetime: {e}. It will remain an object type.")
                    # If conversion fails, it remains object.
                    pass

    # Convert object columns with low cardinality to category
    # Exclude columns that were identified as date-like but failed conversion, they should remain object.
    for col in df.select_dtypes(include='object'):
        if col not in processed_date_cols or pd.api.types.is_datetime64_any_dtype(df[col]):
            # If it's not a processed date col OR it was successfully converted to datetime (which means it's not object anymore)
            # This check might be redundant for the second part if select_dtypes(include='object') is efficient
            if df[col].nunique() < 50:
                # Additional check: ensure it's not a date-like column that failed conversion
                is_failed_date_col = any(x in col.lower() for x in ['date', 'time', 'timestamp']) and col in processed_date_cols and not pd.api.types.is_datetime64_any_dtype(df[col])
                if not is_failed_date_col:
                    logger.info(f"Converting column '{col}' to category type.")
                    df[col] = df[col].astype('category')
                else:
                    logger.info(f"Skipping category conversion for failed date column '{col}'.")
